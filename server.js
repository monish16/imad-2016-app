...
app.post('/create-user', function (req, res) {
   ...
   pool.query('INSERT INTO "user" (username, password, link) VALUES ($1, $2, $3)', [username, dbString, link], function (err, result) {
      if (err) {
   ...
   pool.query('SELECT * from article where title = $1', [article], function (err, result) {
            if (err) {
                ...
                if (result.rows.length === 0) {
                    ...
                    pool.query('INSERT INTO article_likes (article_id,user_id) VALUES ($1,$2)', [articleId,author],
                        function (err, result) {
                            if (err) {
                                res.send('hi');
                            } else {
